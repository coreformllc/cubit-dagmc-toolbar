...
def main():
    ...
    for (gid, name) in dagmc_groups().items():
        ...
        checkboxes[-1].setCheckState(Qt.Checked)
    ...
    selectAllWidget.clicked.connect(lambda _: [c.setCheckState(Qt.Checked) for c in checkboxes])
    deselectAllWidget.clicked.connect(lambda _: [c.setCheckState(Qt.Unchecked) for c in checkboxes])
